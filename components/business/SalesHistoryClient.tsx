...
export default function SalesHistoryClient({ initialSales }: Props) {
  ...
  return (
    <div className="p-4 sm:p-6 max-w-[1600px] mx-auto space-y-6 bg-zinc-50 min-h-full">
      <div>
        <h1 className="text-xl font-bold text-zinc-900 tracking-tight">Sales History</h1>
        <p className="text-xs text-zinc-500 mt-0.5">In-person POS transactions for your shop.</p>
      </div>
          ...
          <table className="w-full text-left text-xs text-zinc-600">
            ...
            <tbody className="divide-y divide-zinc-100 font-medium">
              {initialSales.length === 0 ? (
                <tr>
                  <td colSpan={4} className="p-8 text-center text-zinc-400 italic bg-white">
