...
import { voidTransactionAction } from '@/app/(app)/sales-history/actions'
...
export default function PinVerificationModal({ saleId, onSuccess, onCancel }: ModalProps) {
  ...
  const handleAuthorizationSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    if (pin.length !== 4) return setError('Manager passcode must contain exactly 4 numeric characters.')
  ...
  return (
    <div className="fixed inset-0 z-50 bg-zinc-950/60 flex items-center justify-center p-4 backdrop-blur-xs">
      <div className="bg-white border border-zinc-200 shadow-2xl rounded-2xl max-w-sm w-full p-6 text-center animate-scale-in">
        ...
        <h3 className="text-sm font-bold text-zinc-900">Elevated Verification Authorization</h3>
        <p className="text-[11px] text-zinc-400 mt-0.5 mb-4">Input administrative master access key configuration.</p>
        ...
        <form onSubmit={handleAuthorizationSubmit} className="space-y-4">
          ...
          <div className="flex gap-2 pt-2">
            ...
            >
              {processing ? 'Processing...' : 'Authorize Vault Void'}
            </button>
          </div>
        </form>
      </div>
