...
export default function PosClient({ initialProducts, categories, ingredients }: Props) {
  ...
  return (
    <div className="flex h-[calc(100vh-4rem)] max-w-[1600px] mx-auto overflow-hidden bg-zinc-50 border-t border-zinc-200">
      
      {/* LEFT PANE: Product Browser */}
      <div className="w-7/12 flex flex-col p-5 h-full overflow-hidden border-r border-zinc-200">
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between mt-4 pt-2 border-t border-zinc-50 w-full">
                      ...
                      {/* FIX: Swapped out broken ₱{...} string interpolator syntax */}
      ...
      {/* RIGHT PANE: Cart Checkout Panel */}
      <form onSubmit={handleCheckout} className="w-5/12 flex flex-col h-full bg-white">
        ...
        <div className="flex-1 overflow-y-auto divide-y divide-zinc-100 px-4">
          {cart.length === 0 ? (
            ...
            cart.map(item => (
              <div key={item.product.id} className="py-3.5 flex items-center justify-between gap-3">
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-xs text-zinc-400 mt-0.5">₱{Number(item.product.selling_price).toFixed(2)} each</p>
                </div>
        ...
        {/* Calculation Summary and Payment Input */}
