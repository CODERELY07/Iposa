'use client'

import { useState, useTransition } from 'react'
import { toast } from 'sonner'
import { reviewAffiliateAction } from '@/app/admin/affiliates/actions'
import type { Affiliate } from '@/lib/types/marketplace'
import { Card } from '@/components/ui/card'
import { Button } from '@/components/ui/button'
import { Input } from '@/components/ui/input'
import { Badge } from '@/components/ui/badge'
import { ApplicationStatusBadge } from '@/components/marketplace/StatusBadge'
import { Link2 } from 'lucide-react'

export default function AffiliateReviewClient({ affiliates }: { affiliates: Affiliate[] }) {
  const [isPending, startTransition] = useTransition()
  const [busyId, setBusyId] = useState<string | null>(null)
  const [rejectingId, setRejectingId] = useState<string | null>(null)
  const [reason, setReason] = useState('')

  function approve(id: string) {
    setBusyId(id)
    startTransition(async () => {
      const result = await reviewAffiliateAction(id, 'approved')
      if (!result.success) toast.error(result.message)
      setBusyId(null)
    })
  }

  function reject(id: string) {
    setBusyId(id)
    startTransition(async () => {
      const result = await reviewAffiliateAction(id, 'rejected', reason)
      if (!result.success) toast.error(result.message)
      setBusyId(null)
      setRejectingId(null)
      setReason('')
    })
  }

  return (
    <div className="space-y-4">
      {affiliates.length === 0 && (
        <div className="flex flex-col items-center gap-2 rounded-2xl border border-dashed bg-card py-16 text-center">
          <span className="flex size-12 items-center justify-center rounded-2xl bg-gradient-brand-soft">
            <Link2 className="size-5 text-primary" />
          </span>
          <p className="text-sm text-muted-foreground">No affiliate applications yet.</p>
        </div>
      )}

      {affiliates.map(a => (
        <Card key={a.id} className="p-4">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            <div className="min-w-0">
              <div className="flex items-center gap-2">
                <h3 className="text-sm font-bold text-foreground">{a.full_name}</h3>
                <ApplicationStatusBadge status={a.status} />
              </div>
              <Badge variant="outline" className="mt-1 font-mono">{a.code}</Badge>
              {a.payout_method && (
                <p className="mt-2 text-sm text-foreground">
                  {a.payout_method}{a.payout_details ? ` — ${a.payout_details}` : ''}
                </p>
              )}
              {a.status === 'rejected' && a.rejection_reason && (
                <p className="mt-2 text-xs text-destructive">Reason: {a.rejection_reason}</p>
              )}
              <p className="mt-2 text-[11px] text-muted-foreground">
                Submitted {new Date(a.created_at).toLocaleString()}
              </p>
            </div>

            {a.status === 'pending' && (
              <div className="flex shrink-0 flex-col items-start gap-2 sm:items-end">
                <div className="flex gap-2">
                  <Button size="sm" className="bg-emerald-600 text-white hover:bg-emerald-700" disabled={isPending && busyId === a.id} onClick={() => approve(a.id)}>
                    Approve
                  </Button>
                  <Button size="sm" variant="destructive" onClick={() => setRejectingId(rejectingId === a.id ? null : a.id)}>
                    Reject
                  </Button>
                </div>
                {rejectingId === a.id && (
                  <div className="flex w-64 items-start gap-2">
                    <Input value={reason} onChange={e => setReason(e.target.value)} placeholder="Reason (optional)" className="flex-1 text-xs" />
                    <Button size="sm" variant="destructive" disabled={isPending && busyId === a.id} onClick={() => reject(a.id)}>
                      Confirm
                    </Button>
                  </div>
                )}
              </div>
            )}
          </div>
        </Card>
      ))}
    </div>
  )
}
